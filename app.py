...
<style>
    ...
    * { font-family: 'Inter', sans-serif; }
    ...
    .hero-title { font-size: 3rem; font-weight: 700; margin: 0; text-shadow: 2px 2px 4px rgba(0,0,0,0.2); }
...
try:
    ...
    class AppDatabase:
        ...
        def update_profile(self, data):
            ...
            if uid in st.session_state.mock_db.get("users", {}):                st.session_state.mock_db["users"][uid].update(data)
                return True
            return False
...
# ============================================================
# 5. FOOD SERVICE (Mínimo)
# ============================================================
FALLBACK_FOODS = {
    "arroz_branco": {"name": "Arroz Branco", "category": "almoco_jantar", "calories": 130, "protein": 2.5, "carbs": 28, "fat": 0.3, "portion": "100g"},
    "feijao_preto": {"name": "Feijão Preto", "category": "almoco_jantar", "calories": 77, "protein": 4.5, "carbs": 14, "fat": 0.5, "portion": "100g"},
    ...
    "ovo_cozido": {"name": "Ovo Cozido", "category": "cafe_manha", "calories": 145, "protein": 13, "carbs": 1.5, "fat": 9.5, "portion": "1 unidade"},    "pao_frances": {"name": "Pão Francês", "category": "cafe_manha", "calories": 300, "protein": 8, "carbs": 58, "fat": 3, "portion": "1 unidade"},
...
class FoodService:
    ...
    def get_categories(self):
        return [{"name": "Café da Manhã", "code": "cafe_manha"}, {"name": "Almoço", "code": "almoco_jantar"}, {"name": "Lanche", "code": "lanche"}, {"name": "Jantar", "code": "almoco_jantar"}, {"name": "Ceia", "code": "ceia"}]
    ...
    def search_foods(self, term, cat=None):
        ...
        return [v for v in FALLBACK_FOODS.values() if (not cat or v.get("category") == cat) and (not term or term in v["name"].lower())]
...
class NutritionService:
    ...
    def calculate_daily_goal(self, tmb, act="moderate", goal="lose"):
        ...
        return max(1200, maintenance - 500) if goal == "lose" else maintenance + 300 if goal == "gain" else maintenance
    def get_daily_summary(self, date_str=None):
        if not date_str: date_str = str(date.today())
        meals = self.db.get_meals_by_date(date_str)
        return {"calories": sum(int(m.get("calories", 0)) for m in meals), "protein": sum(float(m.get("protein", 0)) for m in meals), "carbs": sum(float(m.get("carbs", 0)) for m in meals), "fat": sum(float(m.get("fat", 0)) for m in meals), "count": len(meals), "meals": sorted(meals, key=lambda x: x.get("meal_time", "00:00"))}
    ...
    def register_food(self, food_id, quantity, meal_time=None):
        ...
        meal_data = {"food": food["name"], "quantity": float(quantity), "calories": int(food["calories"] * quantity), "protein": round(food.get("protein", 0) * quantity, 1), "carbs": round(food.get("carbs", 0) * quantity, 1), "fat": round(food.get("fat", 0) * quantity, 1), "meal_date": str(date.today()), "meal_time": meal_time}
...
class GamificationSystem:
    ...
    def calculate_streak(self):
        meals = self.db.get_meals(days=30)        if not meals: return 0
        ...
        for i in range(len(date_objs) - 1, 0, -1):
            if (date_objs[i] - date_objs[i-1]).days == 1: streak += 1
            else: break
        return streak
...
def init_services():
    db = AppDatabase()
    return {"db": db, "nutrition": NutritionService(db), "gamification": GamificationSystem(db), "payment": PaymentService(), "foods": FoodService()}

# ============================================================
# 8. CARREGAR DADOS DEMO
# ============================================================
def load_demo_data():
    """Carrega dados de exemplo para o modo demo parecer real."""
    ...
    db = st.session_state.get("_services", {}).get("db")
    if not db:
        return
    ...
    demo_meals = [
        ...
        {"food": "Feijão Preto", "calories": 100, "protein": 6, "carbs": 18, "fat": 0.5, "meal_time": "12:35", "days_ago": 0},        {"food": "Peito de Frango", "calories": 180, "protein": 35, "carbs": 0, "fat": 4, "meal_time": "12:40", "days_ago": 0},
...
# ============================================================
# 9. TELA INICIAL (LANDING PAGE)
# ============================================================
def render_landing():
    """Tela inicial profissional para protótipo."""
    ...
    with c2:
        if st.button("🎮 Experimentar Demo", use_container_width=True, type="primary"):
            demo_user = {
                ...
                "password": "demo",
                "name": "Usuário Demo",
                "plan": "pro",  # Demo tem plano pro para mostrar features
                "current_weight": 75.0,
                "goal_weight": 70.0,
                "height": 170,
                "age": 30,
            ...
            st.session_state.mock_db["users"]["demo@emagresim.com"] = demo_user            st.session_state.user = demo_user
    ...
    with c1: st.metric("Usuários Ativos", "10,000+")
    with c2: st.metric("Refeições Registradas", "500,000+")
    with c3: st.metric("Média de Perda", "4.5 kg/mês")
    with c4: st.metric("Satisfação", "98%")
...
def render_sidebar(services):
    ...
    with st.sidebar:
        ...
        st.markdown("---\n**🧭 Menu**")
        pages = [("🏠 Início", "home"), ("📊 Dashboard", "dashboard"), ("🍴 Registrar Refeição", "meals"), ("⚖️ Evolução Peso", "weight"), ("📈 Análise", "analysis"), ("👤 Perfil", "profile")]
...
# ============================================================
# 11. VIEWS PRINCIPAIS (Resumidas para caber)
# ============================================================
def render_home(services, user):
    ...
    c1, c2, c3, c4 = st.columns(4)
    with c1: st.markdown(f'<div class="data-card"><div class="data-value">{summary.get("calories", 0)}</div><div class="data-label">🔥 Calorias</div></div>', unsafe_allow_html=True)
    with c2: st.markdown(f'<div class="data-card"><div class="data-value">{streak}</div><div class="data-label">🔥 Dias</div></div>', unsafe_allow_html=True)
    with c3: st.markdown(f'<div class="data-card"><div class="data-value">{w:.1f}</div><div class="data-label">️ Peso</div></div>', unsafe_allow_html=True)
    with c4: st.markdown(f'<div class="data-card"><div class="data-value">{w - goal:+.1f}</div><div class="data-label">🎯 Meta</div></div>', unsafe_allow_html=True)
    ...
    st.markdown(f"**Meta diária:** {meta} kcal")
    st.markdown(f"""<div class="metric-card"><div style="display:flex;justify-content:space-between;margin-bottom:0.5rem;"><span>Progresso</span><span>{percent}%</span></div><div style="background:#e2e8f0;border-radius:9999px;height:0.75rem;overflow:hidden;"><div style="background:linear-gradient(90deg,#667eea,#764ba2);height:100%;width:{percent}%;transition:width 0.5s;"></div></div></div>""", unsafe_allow_html=True)
...
def render_dashboard(services, user):
    ...
    summary = services["nutrition"].get_daily_summary()
    c1, c2, c3 = st.columns(3)
    with c1: st.metric("Calorias Hoje", f"{summary.get('calories', 0)} kcal")    with c2: st.metric("Proteínas", f"{summary.get('protein', 0):.1f}g")
    with c3: st.metric("Refeições", summary.get('count', 0))
...
def render_meals(services, user):
    ...
    categoria = st.selectbox("Tipo de Refeição", ["Café da Manhã", "Almoço", "Lanche", "Jantar", "Ceia"])
    hora = st.time_input("Horário", value=datetime.now().time())
    
    alimentos = services["foods"].get_foods_by_category("cafe_manha" if "Café" in categoria else "almoco_jantar" if "Almoço" in categoria or "Jantar" in categoria else "lanche")
    if alimentos:
        opcoes = {f"{a['name']} • {a.get('portion', '100g')}": a for a in alimentos}
        sel = st.selectbox("Alimento", list(opcoes.keys()), placeholder="Digite para buscar...")
        if sel:
            qtd = st.number_input("Quantidade", 0.1, 10.0, 1.0, 0.1)
            if st.button("✅ Salvar Refeição", type="primary", use_container_width=True):
                ok, nome, cal = services["nutrition"].register_food(opcoes[sel], qtd, hora.strftime("%H:%M"))
                if ok:
...
def render_weight(services, user):
    ...
    if st.button("💾 Salvar", type="primary"):
        services["db"].save_weight({"weight": peso, "log_date": str(date.today()), "user_id": user.get("email")})
        user["current_weight"] = peso
        st.session_state.user = user
        st.success("Peso registrado!"); st.rerun()
    
    df = services["db"].get_weights(days=30)
    if not df.empty:
        try:
            import plotly.graph_objects as go
            fig = go.Figure(go.Scatter(x=df['log_date'], y=df['weight'], mode='lines+markers'))
            st.plotly_chart(fig, use_container_width=True)
        except: pass

def render_analysis(services, user):
    st.title("📈 Análise")
    st.info("📊 Dados analíticos em desenvolvimento")

def render_profile(services, user):    st.title("👤 Perfil")
    with st.form("perfil"):
        nome = st.text_input("Nome", user.get("name", ""))
        peso = st.number_input("Peso", 30.0, 250.0, float(user.get("current_weight", 70.0)))
        ...
        if st.form_submit_button("💾 Salvar"):
            user.update({"name": nome, "current_weight": peso, "height": altura, "age": idade})
            st.session_state.user = user
...
def main():
    ...
    st.session_state._services = services  # Para carregar demo data
    
    if st.session_state.user is None:
        render_landing()
        return
    
    # Carrega dados demo se for o usuário demo
    ...
    elif page == "dashboard": render_dashboard(services, st.session_state.user)
    elif page == "meals": render_meals(services, st.session_state.user)
    elif page == "weight": render_weight(services, st.session_state.user)
    elif page == "analysis": render_analysis(services, st.session_state.user)
    elif page == "profile": render_profile(services, st.session_state.user)
