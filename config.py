# app.py - EmagreSim v14.0 COMPLETO COM AVATAR INTERATIVO
...
try:
    ...
    # Funções fallback para quando avatar_maker não existe
    def tela_avatar():
        ...
        if genero == "Masculino":
            avatares = {
                "joao": {"nome": "João", "emoji": "👨", "cor": "#4A90D9"},
                "carlos": {"nome": "Carlos", "emoji": "🧔", "cor": "#E67E22"},
                "pedro": {"nome": "Pedro", "emoji": "👦", "cor": "#2ECC71"},
            }
        else:
            avatares = {
                "maria": {"nome": "Maria", "emoji": "👩", "cor": "#E84393"},
                "ana": {"nome": "Ana", "emoji": "👧", "cor": "#F39C12"},
                "julia": {"nome": "Julia", "emoji": "💁", "cor": "#9B59B6"},
            }
        ...
        for i, (key, data) in enumerate(avatares.items()):
            with cols[i]:
                st.markdown(f'<div style="background: {data["cor"]}; width: 100px; height: 100px; border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto;"><span style="font-size: 3rem;">{data["emoji"]}</span></div>', unsafe_allow_html=True)
    ...
    def mostrar_avatar():
        if "avatar_data" in st.session_state:
            ...
            if humor == "feliz":
                emoji_expressao = "🎉"
                animacao = "style='animation: bounce 0.5s ease infinite;'"
            elif humor == "triste":
                emoji_expressao = "😔"
                animacao = ""
            else:
                emoji_expressao = emoji
                animacao = ""
            
            st.markdown(f'''
            <div style="display: flex; justify-content: center;">
                <div style="background: {cor}; width: 120px; height: 120px; border-radius: 50%; display: flex; align-items: center; justify-content: center; {animacao}">
                    <span style="font-size: 3.5rem;">{emoji_expressao}</span>
                </div>
            </div>
            <style>
            @keyframes bounce {{
...
def pagina_dashboard():
    ...
    # Simular registro de peso (para testar a celebração)
    ...
    with st.expander("🏆 Desafio da Semana", expanded=True):
        desafios = [
            ...
            ("🍽️ Não pule refeições por 3 dias", 80),
        ]
