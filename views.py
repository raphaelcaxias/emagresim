    ...
    with tab1:
                ...
                if email and password:
                    ...
                    if email in users and users[email].get("password") == password:
                        st.session_state.user = {
                            ...
                            "is_demo": users[email].get("is_demo", False),
        ...
        with col_demo:
            if st.button("🎮 Modo Demo", use_container_width=True):
                st.session_state.user = {
                    "email": "demo@emagresim.com", "name": "Usuário Demo",
                    "is_demo": True, "onboarded": False, "plan": "free"
                }
                st.success("Entrando em modo demo...")
                st.rerun()
            ...
            if new_name and new_email and new_password:
                if new_password == confirm_password:
                    ...
                    if new_email in users:
                        ...
                        st.session_state.mock_db["users"][new_email] = {
                            "password": new_password, "name": new_name,
                            "email": new_email, "is_demo": False
                        }
...
def render_onboarding(nutrition, user):
    ...
    with col_start:
        if st.button("🚀 Começar Jornada", use_container_width=True, type="primary"):
            user.update({
...
def render_weight(nutrition, user):
    ...
    with col2:
        ...
        if not weights_df.empty:
            ...
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=weights_df['record_date'] if 'record_date' in weights_df.columns else weights_df['date'], y=weights_df['val'], mode='lines+markers', name='Peso', line=dict(color='#667eea', width=3), marker=dict(size=8, color='#764ba2')))
            ...
            with st.expander("📋 Ver todos os registros"):
                st.dataframe(weights_df.sort_values('record_date' if 'record_date' in weights_df.columns else 'date', ascending=False), use_container_width=True)
        else:
...
def render_behavior(behavior, user):
    ...
    if achievements:
        ...
        for idx, ach in enumerate(achievements):
            with cols[idx % 3]:
                st.markdown(f"""<div class="metric-card"><div style="font-size: 2rem;">️</div><div style="font-weight: 600;">{ach.get('title', 'Conquista')}</div><div style="font-size: 0.75rem; color: #6b7280;">{ach.get('date', '')}</div></div>""", unsafe_allow_html=True)
    else:
