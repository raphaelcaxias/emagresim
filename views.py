...
def load_css():
    try:
        ...
        # Fallback inline se o arquivo não for encontrado
        st.markdown('<style>.metric-card{background:white;border-radius:16px;padding:1rem;text-align:center}</style>', unsafe_allow_html=True)
...
def render_meals(nutrition, behavior, user):
    ...
    with tab3:
        ...
        meals = nutrition.db.get_meals(days=30)
        emotion_analysis = behavior.get_emotion_analysis(meals)
        if emotion_analysis["total"] > 0:
            col1, col2 = st.columns(2)
            with col1:
                st.markdown(f"""<div class="metric-card"><div class="metric-value">{emotion_analysis['total']}</div><div class="metric-label">Total de refeições (30 dias)</div></div>""", unsafe_allow_html=True)
            with col2:
                if emotion_analysis["most_common"]:
                    st.markdown(f"""<div class="metric-card"><div class="metric-value">🎭 {emotion_analysis['most_common']}</div><div class="metric-label">Emoção mais frequente</div></div>""", unsafe_allow_html=True)
            if emotion_analysis["by_emotion"]:
                fig = px.pie(values=list(emotion_analysis["by_emotion"].values()), names=list(emotion_analysis["by_emotion"].keys()), title="Distribuição por Estado Emocional", color_discrete_sequence=px.colors.qualitative.Set3)
                st.plotly_chart(fig, use_container_width=True)
        else:
...
def render_subscription(payment, user):
    ...
    if user.get("plan") == "pro":
        ...
        with col1:
            if st.button("🔄 Gerenciar Assinatura"):
                st.info("Redirecionando para o portal do cliente...")
        with col2:
            if st.button("📞 Suporte Prioritário"):
                st.info("Em breve você receberá um e-mail com o contato do suporte.")
        return
    ...
    # ✅ FIX: st.query_params retorna string direta
